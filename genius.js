...
(function () {
  ...
  async function selectMaxBalanceToken() {
    ...
    if (savedTab) {
      ...
      UI.logSwap("未找到 Saved 标签，尝试直接选择");
    }
  ...
  async function startSwapLoop() {
    ...
    UI.logSwap('🚀 Bot started. 区间: ' + (SWAP_CONFIG.waitRandomMin/10000) + 's - ' + (SWAP_CONFIG.waitRandomMax/10000) + 's');
    ...
    while (isSwapRunning) {
      try {
        ...
        if (confirmClicked) {
          ...
          if (hasSameAssetError()) {
            await fixSameAssetSelection();
          } else {
